# Bootcamp IA — Dia 3: exemplos práticos

Este repositório reúne os materiais de apoio para o **Dia 3 do Bootcamp de IA**, com exemplos usados para mostrar como transformar uma conversa com IA em uma entrega clara, validável e reutilizável.

A ideia não é ensinar um “prompt perfeito”. A ideia é praticar um fluxo simples:

```text
intenção vaga → contexto → mini-spec → plano → validação → artefato → handoff
```

## Para quem é este material

Este material foi organizado para funcionar tanto para pessoas **não técnicas** quanto para pessoas **técnicas**.

- Se você não é técnico: abra a pasta do exemplo, anexe os arquivos da pasta `dados/` no chat de IA e cole os prompts em sequência.
- Se você é técnico: além de usar os prompts, você pode abrir os CSVs, comparar os dados, adaptar os critérios e criar novos exemplos a partir da mesma estrutura.

## Exemplos disponíveis

| Exemplo | Tema | O que pratica |
|---|---|---|
| `01-camila-inbox-vida-adulta` | Organização pessoal | Transformar pendências soltas em plano possível |
| `02-bruno-reuniao-cliente-aurora-pay` | Trabalho / cliente | Preparar reunião com contexto, riscos e decisões |
| `03-lucas-semana-possivel` | Rotina semanal | Planejar uma semana realista considerando energia e imprevistos |

## Como usar em aula

1. Escolha um exemplo.
2. Abra a pasta do exemplo.
3. Leia o `README.md` daquele exemplo.
4. Anexe os arquivos da pasta `dados/` na ferramenta de IA.
5. Copie e cole os prompts do arquivo `prompts.md`, um por vez.
6. Compare as respostas com os critérios do próprio prompt.
7. Ajuste o contexto, as restrições ou o formato quando necessário.

## O que este repositório contém

Cada exemplo possui:

```text
README.md      → explicação do cenário e modo de uso
prompts.md     → sequência de prompts para usar no chat
dados/         → arquivos mock/fake para simular contexto real
```

Os dados são fictícios e foram criados apenas para fins didáticos.

## O que este repositório não contém

Este repositório não inclui as respostas prontas da IA.

Isso é intencional: o objetivo é que cada aluno execute os prompts, observe os resultados, critique as respostas e pratique a melhoria da conversa.

## Fluxo praticado

```text
1. Pedido vago
2. Contexto inicial
3. Mini-spec
4. Plano ou estrutura
5. Validação
6. Artefato final
7. Handoff / próximo passo
```

## Canvas rápido

Use este canvas antes de pedir uma entrega para a IA:

```text
1. Intenção vaga:
2. Contexto:
3. Objetivo:
4. Restrições:
5. Fora de escopo:
6. Critérios de sucesso:
7. Formato da resposta:
8. Como validar:
```

## Licença e uso

Material criado para fins educacionais no contexto do Bootcamp de IA.

Você pode adaptar os exemplos para estudo, aula, workshop ou prática individual.
